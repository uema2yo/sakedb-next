...
console.log(token, validateEmailExists.result, validateEmailExists.message);

  return (
    <>
      <MainHeader title="アカウント登録">
        <p>新規ユーザー登録を行います。</p>
        <p>※ 既にアカウントをお持ちの方は<Login variant="link_inline" size="inline" />してください。</p>
      ...
      {validateEmailExists.result ? (
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSignup)}>
            ...
            <ul>
              <li>※ 利用規約に同意しないと、アカウントを作成できません。</li>
              <p>※ ユーザー ID は、半角英数と「_」以外は使用できません。</p>
              <p>※ パスワードは、半角英数記号による最低 8 文字を設定してください。
              </p>
              <p>
                ※ 確認用パスワードに同じパスワードを再入力してください。
              </p>
              <p>※ ユーザー ID とパスワードは、ログイン時に必要になります。</p>
              <p>※ ユーザー ID は、ウェブ上で一般公開されます。</p>
            </ul>
              <div>
                <Button
                  variant="primary"
        ...
        <>
          {validateEmailExists.result === false && (
            ...
            <p>または、<Login variant="link_inline" size="inline" />フォームからログインしてください。</p>
            </>
          )}
